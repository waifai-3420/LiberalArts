...
class Game:
    ...
    def setup_game(self):
        ...
        for _ in range(len(node_ids) * 2):
            n1_id = random.choice(node_ids)
            n2_id = random.choice(node_ids)
            if n1_id == n2_id:
                continue
            if (n1_id, n2_id) in connections or (n2_id, n1_id) in connections:
                continue
            ...
            # エッジ画像も一緒に渡す
            self.graph.add_edge(node1.id, node2.id, distance, self.edge_image) 
            connections.add((n1_id, n2_id))
    ...
    def draw(self):
        ...
        if self.game_state == 'PLANNING' and len(self.route_planning_nodes) > 1:
            for i in range(len(self.route_planning_nodes) - 1):
                start_node = self.graph.nodes[self.route_planning_nodes[i]]
                end_node = self.graph.nodes[self.route_planning_nodes[i+1]]
                pygame.draw.line(self.screen, BLUE, (start_node.x, start_node.y), (end_node.x, end_node.y), LINE_WIDTH + 2)
